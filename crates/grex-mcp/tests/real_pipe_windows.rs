//! L2 real-pipe per-OS guard — **Windows** target.
//!
//! Owned by `feat-m7-2-mcp-test-harness` Stage 3. Compiled only on
//! Windows; the parallel `real_pipe_linux.rs` / `real_pipe_macos.rs`
...
//!
//! 1. **Real OS pipe back-pressure.** Windows named-pipe / anonymous-pipe
//!    buffers default to ~4 KiB; a >64 KiB response forces multiple
//!    write-block / read-drain cycles. `duplex` is a `BytesMut`-backed
//!    in-process channel — it never blocks on buffer pressure.
//! 2. **Stderr-handle independence.** `duplex` has no concept of stderr;
//!    closing the client's stderr cannot affect a server reading stdin /
//!    writing stdout. The real subprocess case must hold even when the
//!    parent abandons stderr (a common pattern in MCP host shims that
//!    only multiplex stdout/stdin).
//!
//! Both guarantees come from `inst/mcp.md` §"Stdio discipline" and
...
// Windows surprise budget — pipe buffer flushing under back-pressure
// plus ChildStdout `read_line` polling latency runs higher than the
// POSIX hosts. `MEMORY.md` records 500 ms for cancellation; we mirror
// that envelope here for response-drain assertions so the cross-OS
// budget table lives in one place.
...
fn drive_init(stdin: &mut std::process::ChildStdin, id: u32) {
    let init = format!(
        r#"{{"jsonrpc":"2.0","id":{id},"method":"initialize","params":{{"protocolVersion":"2025-06-18","capabilities":{{}},"clientInfo":{{"name":"grex-mcp-real-pipe-windows","version":"0.0.1"}}}}}}"#,
    );
...
fn large_response_crosses_pipe_buffer() {
    // Each `tools/list` response is roughly 2–6 KiB on the m7-1 stub
    // surface (11 tools × name+description+JSON-Schema). 32 frames
    // comfortably cross the 64 KiB target with margin to spare while
    // staying well under any 1 MiB Windows ChildStdout buffer ceiling.
    const BURST: u32 = 32;
    ...
    // Fire BURST requests with no intervening reads — this is what
    // forces the OS pipe buffer to fill; if the rmcp framer stalled
    ...
    // Drain + assert all 32 responses arrive intact and in order;
    // the helper carries the per-frame JSON / id / monotonicity asserts.
...
/// receive responses.
///
/// Mechanic on Windows: spawning the child with `Stdio::null()` for
/// stderr discards the server's stderr writes into the OS null device.
/// This is the closest analogue to a host that opens stdin/stdout pipes
/// but ignores stderr — the case `inst/mcp.md` calls out under
/// "Stdio discipline" ("server MUST tolerate stderr being unread or
/// redirected to /dev/null"). If the server panicked on a full stderr
/// buffer or aborted on a stderr-write `BrokenPipe` error the
/// subsequent `tools/list` would either time out (process still alive
/// but wedged) or surface EOF (process gone). Both manifest as a
/// failure of `read_until_id`.
///
/// We do NOT close stderr on an already-running child via Win32
/// `CloseHandle` — that races against the rmcp service spawning its
...
fn client_stderr_close_does_not_panic_server() {
    ...
    // First post-init request — must succeed; this proves the server's
    ...
    // Clean exit confirms the server's serve loop unwound normally
    // rather than via panic/abort.
