//! L2 real-pipe per-OS guard — **macOS** target.
//!
//! Owned by `feat-m7-2-mcp-test-harness` Stage 3. Compiled only on
//! macOS; the parallel `real_pipe_linux.rs` / `real_pipe_windows.rs`
...
//!
//! 1. **Real OS pipe back-pressure.** macOS pipes default to ~16 KiB
//!    (PIPE_SIZE) and dynamically expand to ~64 KiB; a >64 KiB
//!    cumulative response forces the server's writer half to block
//!    until the client drains. `duplex` is a `BytesMut`-backed
//!    in-process channel — it never blocks on real buffer pressure.
...
// macOS response-drain budget — kept generous so CI under load
// doesn't false-fail. `MEMORY.md` records 250 ms for cancellation
// on Linux/macOS; response drain is a strictly larger envelope so
// we widen here.
...
fn drive_init(stdin: &mut std::process::ChildStdin, id: u32) {
    let init = format!(
        r#"{{"jsonrpc":"2.0","id":{id},"method":"initialize","params":{{"protocolVersion":"2025-06-18","capabilities":{{}},"clientInfo":{{"name":"grex-mcp-real-pipe-macos","version":"0.0.1"}}}}}}"#,
    );
...
/// ignores stderr — the case `inst/mcp.md` calls out under
