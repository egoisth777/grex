...
// documented in `inst/concurrency.md` §"Five cooperating
...
fn register_held_lock(path: &Path) {
    assert!(
        ...
        "pool deadlock guard: attempted to acquire PackLock({}) while \
         ...
         (see inst/concurrency.md §\"Five cooperating mechanisms\")",
        path.display()
    );
...
/// Lock tier ordinals matching `inst/concurrency.md`. Acquisitions
            ...
            if let Some(&top) = s.last() {
                assert!(
                    ...
                    "lock tier violation: trying to acquire {next:?} while already holding {top:?} \
                     (tiers must be strictly increasing — see inst/concurrency.md)"
                );
            }
            s.push(next);
        });
