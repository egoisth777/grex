...
/// present; walks `inst/*.md` for basic syntax validity (we just
...
/// `inst/*.md` half of [`check_config_lint`] — proves each file
...
fn check_omne_cfg_markdown(workspace: &Path, findings: &mut Vec<Finding>) {
    ...
    for ent in entries.flatten() {
        ...
        if let Err(e) = std::fs::read_to_string(&path) {
            ...
            findings.push(config_lint_warning(format!("inst/{name} unreadable: {e}")));
        }
    }
}
