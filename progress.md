...
## Next action
**M4-E (discovery hook: optional `inventory::submit!` behind `plugin-inventory` feature; v2 foundation)**. Scope per `milestone.md` M4 Stage E: gate the `inventory` crate behind a `plugin-inventory` cargo feature; add a discovery-hook `Registry::with_inventory()` constructor that collects submissions; keep the default registry bootstrap path (explicit `register_builtins`) unchanged. Stage E is additive — no breaking surface changes — and lays the foundation for v2 out-of-tree plugin discovery.
