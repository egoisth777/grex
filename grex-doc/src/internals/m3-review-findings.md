...
- `inst/concurrency.md` — updated to document workspace + repo fd-lock contract.
- `inst/manifest.md` — updated to document `ActionStarted` / `ActionCompleted` / `ActionHalted` event brackets.
- `inst/actions.md` — updated to document symlink backup-rollback, `kind: auto` missing-src error, and exec stderr truncation.
